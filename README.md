...
- 分层对话记忆：界面保留最近 200 条消息；模型携带最近 6 轮完整原文，并将更早对话压缩为持久化长期摘要。
- 多会话身份隔离：新建对话时输入员工姓名或选择游客模式，不同会话的历史和长期记忆互不混用。
- 知识库管理：支持预览文档正文、从回答引用直接打开资料，以及确认后删除文档和原始文件。
...
- 已内置：账号登录、退款售后、客户支持 SLA、API 限流、账单发票、数据安全、成员权限、故障响应、套餐配额、数据导出、企业接入和密钥安全等知识资料。

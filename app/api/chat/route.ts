...
export async function POST(request: Request) {
  ...
  const d1 = await db();
  const session = await d1.prepare("SELECT id, display_name AS displayName, mode, account_id AS accountId FROM agent_sessions WHERE id = ?").bind(sessionId).first<{ id: string; displayName: string; mode: string; accountId: string | null }>();
  const actor = await resolveViewer(request, d1);
  ...
  const sameOwner = actor.mode === "guest" ? actor.id === session.id : session.mode === "employee" && (actor.accountId === session.accountId || (!session.accountId && actor.displayName.trim().toLowerCase() === session.displayName.trim().toLowerCase()));
  if (!sameOwner) return Response.json({ error: "该对话为只读历史，不能使用他人身份发送消息" }, { status: 403 });
  if (!session.accountId && actor.accountId) await d1.prepare("UPDATE agent_sessions SET account_id = ? WHERE id = ?").bind(actor.accountId, session.id).run();
